...
class Solution:
    def exist(self, board, word: str) -> bool:
        ...
        n = len(board[0])
        self.flag = False
        # def bfs(board, i, j, word, index, flagmatrix):
        #     if index == len(word):
        #         self.flag = True
        #         return
        ...
        #         x = step[0]+i
        #         y = step[1]+j
        #         if 0<=x<m and 0<=y<n and flagmatrix[x][y]==0 and board[x][y]==word[index]:
        #             flagmatrix[x][y] = 1
        #             bfs(board, x, y, word, index+1, flagmatrix)
        #             flagmatrix[x][y] = 0
        # flagmatrix = [[0 for i in range(n)] for j in range(m)]

        def bfs(board, i, j, word, index):
            if index == len(word):
                self.flag = True
                return
            steps = [(0, 1), (0, -1), (1, 0),(-1, 0)]
            for step in steps:
                x = step[0]+i
                y = step[1]+j
                if 0<=x<m and 0<=y<n and board[x][y]!='' and board[x][y]==word[index]:
                    board[x][y] = ''
                    bfs(board, x, y, word, index+1)
                    board[x][y] = word[index]
                    # 加了这句话一旦找到一个符合该例子的即返回 节省时间
                    if self.flag:
                        return
        ...
        for i in range(m):
            for j in range(n):
                if board[i][j] == word[start]:
                    # flagmatrix[i][j] = 1
                    board[i][j] = ''
                    # bfs(board, i, j, word, start+1, flagmatrix)
                    bfs(board, i, j, word, start+1)
                    # flagmatrix[i][j] = 0
